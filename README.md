...
two results: a **30.168% K15 charged-format estimate** (11.173 bits/weight),
and a **24.967% byte-split format** (12.005 bits/weight) that round-tripped
all 59,509 BF16 tensors bit-for-bit.
...
```bash
uv sync
# verify losslessness + decoded reduction, streamed shard by shard from Hugging Face:
uv run verify.py <org>/<repo>
```

`ALL_BF16_TENSORS_LOSSLESS: true` is the whole point. The streaming verifier
intentionally omits accounting-only layouts that it does not decode.
