...
def roundtrip_bytesplit(raw: bytes, rows: int):
    ...
    bits = (
        n * 4
        + n_esc * 8
        + rows * max(1, math.ceil(math.log2(n_esc + 1)))
        + K * 8
        + n * 8
    )
...
def main():
    ap = argparse.ArgumentParser(
        description="Verify lossless BF16 compression on any model."
    )
    ap.add_argument(
        "--model",
        required=True,
    ...
    ap.add_argument(
        "--layout", choices=["both", "bytesplit", "regroup"], default="both"
    )
    ap.add_argument(
        "--limit", type=int, default=0, help="stop after N BF16 tensors (0 = all)"
    )
    args = ap.parse_args()
    ...
    if do_bs:
        bpw = t["bs_bits"] / w
        print(
            f"  byte-split           : {bpw:6.3f} b/w   -{100 * (1 - bpw / 16):4.1f}%   escapes {100 * t['esc_bs'] / w:.3f}%"
        )
    if do_rg:
        bpw = t["rg_bits"] / w
        print(
            f"  regroup (headline)   : {bpw:6.3f} b/w   -{100 * (1 - bpw / 16):4.1f}%   escapes {100 * t['esc_rg'] / w:.3f}%"
        )
    print()
