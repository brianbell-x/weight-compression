...
def main(K=370):
    ...
    U = u.reshape(R0, C)
    high = (U >> 8).astype(np.uint8); low = (U & 0xFF).astype(np.uint8)
    sign = high >> 7; exp7 = high & 0x7F
    ...
    inr = (off >= 0) & (off < 8)
    code = np.where(inr, (sign.astype(np.int32) << 3) | np.clip(off, 0, 7), 0xF).astype(np.uint8)  # [R0,C]
    esc = ~inr
    ...
    il_idx0 = (il_code0[:, 0::4].astype(np.uint16) | (il_code0[:, 1::4].astype(np.uint16) << 4)
               | (il_code0[:, 2::4].astype(np.uint16) << 8) | (il_code0[:, 3::4].astype(np.uint16) << 12))
    il_low = torch.from_numpy(np.tile(il_low0.view(np.uint32), (K, 1)).reshape(-1)).to(DEV)
    il_idx = torch.from_numpy(np.tile(il_idx0, (K, 1)).reshape(-1)).to(DEV)
    W = torch.from_numpy(u.view(np.int16)).to(DEV).view(torch.bfloat16).reshape(R0, C).repeat(K, 1)
    x = torch.randn(C, device=DEV, dtype=torch.float32); xf = x
    ...
    yb = torch.empty(R0, device=DEV, dtype=torch.float32); yi = torch.empty(R0, device=DEV, dtype=torch.float32)
    ilx = torch.from_numpy(il_idx0.reshape(-1)).to(DEV); ilw = torch.from_numpy(il_low0.view(np.uint32).reshape(-1)).to(DEV)
    ...
    if len(er):
        ...
        rows = torch.from_numpy(er.astype(np.int64)).to(DEV); xcols = xf[torch.from_numpy(ec.astype(np.int64)).to(DEV)]
        corr = torch.zeros(R0, device=DEV).index_add_(0, rows, torch.from_numpy(dw).to(DEV)*xcols)
        yi = yi + corr
    rel = ((yi - yb).abs().max() / (yb.abs().max() + 1e-9)).item()

    # ---- perf (tiled to bandwidth-bound) ----
    cfgs = [(256,4),(512,8),(1024,8),(2048,8),(2688,16),(1024,16),(512,4),(256,2)]
    wb = R*C
    tb, cb = best(lambda B, w: (lambda: k_bf16[grid](y, W, x, R, C, BLOCK=B, num_warps=w)), cfgs)
    ti, ci = best(lambda B, w: (lambda: k_il[grid](y, il_idx, il_low, x, R, C, Q, BASE, BLOCK=B, num_warps=w)), cfgs)
    out = {"gpu": torch.cuda.get_device_name(0), "K": K, "BASE": BASE,
           ...
           "ratio_il_over_bf16": round(ti/tb, 3), "ideal": 0.75, "best_cfg": {"bf16": cb, "il": ci},
           "WIN": bool(ti < tb)}
    print(json.dumps(out, indent=2)); open("/workspace/kernel_v10_result.json", "w").write(json.dumps(out, indent=2))
