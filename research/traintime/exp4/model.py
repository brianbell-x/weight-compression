...
class FeedForward(nn.Module):
    ...
    Default families are pure linear maps: two make_linear maps (fc1, fc2) with a
    GELU between them -- the model owns the nonlinearity so every linear family is
    compared on equal footing.

    A family MAY instead return a self-contained *full FFN* module by setting the
    attribute ``is_full_ffn = True`` on the object returned from the d_model->d_ff
    call. Such a module owns its own nonlinearity AND output projection (it maps
    d_model -> d_model), so the harness uses it ALONE and skips the GELU+fc2 path.
    This is required for families whose whole point is an internal nonlinear
    structure (e.g. a top-k SPARSE wide hidden layer); a pure linear map cannot
    express that. The dense path is unchanged (getattr is False for dense), so the
    dense baseline reproduces exactly.
    """

    def __init__(self, cfg: ModelConfig, gen: torch.Generator):
        super().__init__()
        fc1 = make_linear(cfg.family, cfg.d_model, cfg.d_ff,
                          cfg.matrix_budget, gen)
        if getattr(fc1, "is_full_ffn", False):
            self.full = fc1
            self.fc1 = None
            self.fc2 = None
        else:
