...
# exp2 makes the FFN the BINDING capacity constraint: d_model is small (64) so the
# token/positional embeddings and attention are deliberately under-powered, and
# the only knob that grows with param_budget -- the FFN width d_ff -- carries the
# burden of modelling the hard real-text corpus. Longer sequences (96) give real
# long-range structure for the extra capacity to exploit. Result: dense val loss
# keeps dropping as the budget grows (capacity-bound), unlike exp1.
SEQ_LEN = 96
D_MODEL = 128     # exp4: raised from 64 -> dimensional room for superposition
N_BLOCKS = 2
N_HEADS = 4
BATCH_SIZE = 32
TRAIN_STEPS = 1000
